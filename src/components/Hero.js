...
const Hestonection = ({ scrollToSection }) => {
  ...
  setTimeout(()=>{setCanClick3dBtn(true)}, 4000);
    ...
    <div className="container mx-auto flex flex-col md:flex-row items-center gap-2">
      ...
      <div className="md:w-1/2 w-full flex lg:justify-end relative animation-container">
        ...
        <div
          ...
          <div style={{ width: '100%', height: '100%' }}>
            <ProductViewer playAnimation={playAnimation} setPlayAnimation={setPlayAnimation} canClick3dBtn={canClick3dBtn} />
          </div>
        </div>

        <div className="group absolute bottom-4 flex space-x-4 mr-4">
          <button onClick={togglePlayAnimation} className={`
          border-2
          border-primary
          rounded-full 
          ...
          duration-300
          `}>
            <svg className="size-14" id="group" fill={playAnimation ? '#82ce51' : '#fff'} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000">
              <path id="arrow-left" className="group-hover:scale-85 transition-transform origin-center duration-300" d="M223.07,749.55l-134.3,78.09c23.17,10.04,94.36,11.55,55.26,49.72h-13.27l-95.68-27.02c-9.13-5.58-11.23-9.57-10.25-20.24,1.79-19.49,16.42-67.23,23.02-87.56,4.43-13.62,6.9-27.05,25.53-21.12,23.96,7.62-.18,53.84-2.28,70.93-.43,3.48-.34,3.66,3.23,3.21l129.17-76.61,19.55,30.6Z" />
              <path id="arrow-right" className="group-hover:scale-85 transition-transform origin-center duration-300" d="M793.92,718.99l131.76,76.55c5.77.5,2.21-.94,1.65-3.86-2.64-13.84-15.8-47.58-13.43-58.59,3.46-16.05,28.41-17.42,33.85-1.62l27.63,99.47v4.42c-3.42,2.51-2.64,7.55-5.04,10.46-8.72,10.58-86.76,21.86-103.3,31.53h-11.06c-6.14-4.23-11.59-6.47-12.88-14.95-4.09-26.93,49.32-24.96,65.94-34.77l-136-78.38,20.88-30.27Z" />
              <path id="arrow-up" className="group-hover:scale-85 transition-transform origin-center duration-300" d="M482.32,259.6V114.83c-16.57,9.13-43.21,57.57-64.29,41.06-12.53-9.82-5.48-21.68,2.33-31.16,10.57-12.81,53.01-56.68,65.27-65.15,8.19-5.65,16.9-6.96,25.56-1.23s62.76,60.55,69.56,69.71c5.73,7.72,9.41,20.38.99,27.6-20.74,17.79-48.34-29.32-64.05-38.61v145.87l-35.38-3.32Z" />
              <g className="group-hover:scale-110 transition-transform origin-center duration-300">
                <polygon id="cube-color" fill={!playAnimation ? '#82ce51' : '#fff'} points="500 278.93 223.07 367.48 223.07 724.44 500 821.22 773.04 734.49 782 380 500 278.93" />
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-[10000]">
          <div
            className="bg-white rounded-lg shadow-xl w-[90%] sm:w-[80%] md:w-[70%] lg:w-[50%] max-w-[800px] h-[80vh] relative z-[10001] overflow-hidden flex flex-col pt-12 px-6">
            <button
              onClick={closeModal}
